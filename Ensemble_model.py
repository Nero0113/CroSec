...
class EnsembleModel:
    def __init__(
        ...
        # scipy_matrix = sp.load_npz(sparse_matrix_path)
        # self.sparse_matrix = torch.sparse_coo_tensor(
        #     scipy_matrix.nonzero(),
        #     scipy_matrix.data,
        #     scipy_matrix.shape
        # ).t().to(self.src_model.device).to(torch.float32)
        # 获取 scipy_matrix 的非零索引
        # indices = scipy_matrix.nonzero()
        # 将 indices 转换为 numpy 数组，然后再转换为 tensor
        # indices_tensor = torch.tensor(np.array(indices), dtype=torch.long)
        ...
        #     scipy_matrix.shape
        # ).t().to(self.src_model.device).to(torch.float32)
        self.trg2src = torch.tensor(json.load(open('/home/public_space/yanmeng/zhangjingrui/projects/OFA/emb_map_files/Qwen2.5-0.5b_2_CodeShell-7B/token_maps/one2one_CodeShell-7B2Qwen2.5-0.5b_id.json')), dtype=torch.long, device=self.trg_model.device)
        self.token_map = torch.tensor(
            json.load(open('/home/public_space/yanmeng/lidong/code/one4all/try_EVA_like/mapping_codeshell/token_map_full.json')), dtype=torch.long, device=self.trg_model.device)
        spmat = sp.load_npz('/home/public_space/yanmeng/lidong/code/one4all/try_EVA_like/mapping_codeshell/src2trg_full.npz')
    ...
    def generate(
        ...
        # ---- Starcoder2: FIM prompt formatting ----
        # if "starcoder" in self.trg_model.config.name_or_path.lower() or "starcoder2" in self.trg_model.config.name_or_path.lower():
        ...
        # # ① 统计覆盖率
        # cover = (self.token_map != -1).float().mean().item()
        # print(f"mapped coverage = {cover * 100:.1f}%  (≥20% 通常就足够见效)")
        #
        # # ② 随机抽一个 *有映射* 的 src token
        # mapped_ids = torch.nonzero(self.token_map != -1).squeeze(1).cpu().tolist()
        # tid = np.random.choice(mapped_ids)
        # print("sample mapped src id =", tid, "→ trg id =", self.token_map[tid].item())
        #
        # src_vec = torch.zeros(1, self.sparse_matrix.size(0), device=self.sparse_matrix.device)
        # src_vec[0, tid] = 1
        # mapped = torch.spmm(src_vec, self.sparse_matrix).squeeze(0)
        # print("top-5 probs:", torch.topk(mapped, 5))
        # exit()
